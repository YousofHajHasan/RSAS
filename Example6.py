...
def monitor_shared_data(shared_data):
    ...
    while True:
        ...
        if shared_data.get("Right Now Outside", 0) > 5 and not notified_outside:
            ...
            notified_outside = True
        elif shared_data.get("Right Now Outside", 0) <= 5:
            notified_outside = False

        if shared_data.get("Entered The House", 0) > 5 and not notified_entered:
            send_push_notification("More than 5 people have entered the house. This might be unusual.")
            notified_entered = True  

        elif shared_data.get("Entered The House", 0) <= 5:
            notified_entered = False  

        if current_right_to_left // 2 > last_notified_right_to_left // 2:
            send_push_notification("A new 10 cars have moved from right to left.")
            last_notified_right_to_left = current_right_to_left  
        
        if current_left_to_right // 2 > last_notified_left_to_right // 2:
            send_push_notification("A new 10 cars have moved from left to right.")
            last_notified_left_to_right = current_left_to_right  
